	...
	while (1)
	{
		...
		if (ct < timer)									/* wait time over								*/
		{
			if (( triperrorcount < MAX_TRIP_ERROR)
				|| (insp & INSP_ON)						/* or Inspection is on							*/
				...
				|| (term_active))						/* or Terminal mode 							*/
			{
				...
				if ((driveerror & D_BRAKE_NOTON) && ((insp & INSP_ON) || (em_op & INSP_ON)))
					driveerror &= ~D_BRAKE_NOTON;
				if ((driveerror & D_BRKCONT_NOTON) && ((insp & INSP_ON) || (em_op & INSP_ON)))
				...
				if (((driveerror & D_FXCONT_NOTON) && ((insp & INSP_ON) || (em_op & INSP_ON))) ||		//检修状态认为干预消除
						(!p.fx_active))
		...
		if (!wait_time_over)						/* error wait time not over							*/
		{
			if (errortimer < timer)					/* time over										*/
			{
				...
				if ((door_zone & POS_SGM) && (level != 0xFF) && (!(em_op & INSP_ON)) && (!(insp & INSP_ON)))
				{
					opendoorflag = 1;
