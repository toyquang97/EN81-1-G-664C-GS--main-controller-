...
//����Ӧ����Դ�ĳ��빦��(����ϵ�Դ���磬���ݲ�ֹͣ)		
	...
	if ((calls_off) || (triperrorcount >= MAX_TRIP_ERROR) || (door_short_conn) ||
			(ucmp_state) || (brake_value == BRAKE_NO_GOOD) || (bTest_brake))
	...
	if (toggle)
	{
		...
		{												/* set gong only in normal state	*/
			...
			if ((arrival_ind) && (gongoutputenable) && 
					(!(bFunc_flag & FUNC_ARRIVEGONG_TIMEIN) || !(p.cooperate_sel1 & COOP_ARRIVE_NIGHTGONG)))	/* new opened doors in arrival state			*/
			{
...
//���ӵ�վ��ҹ��ʱ����ж�
		if(p.cooperate_sel1 & COOP_ARRIVE_NIGHTGONG)
			{
	...
	if(p.canbus_check)
		{//���й����в�����can����
			if ((canerrorstate & (TXA_SW_OVERRUN | RXA_SW_OVERRUN | CANA_BUSOFF | SXA_SW_OVERRUN))
				...
				{
					if((can_init_timer < timer) && (!can_init_start)	&& ((drive_state == D_STOPPED) || !(insp & INSP_ON)))
						{
							can_init_start = 1;
