...
BYTE set_park_state (void)
{
		...
		emp_power )
	{
		...
		parkstate = p.doortable [adt][level];
		if((p.cooperate_sel1 & COOP_THROUGH_DOOR) && (p.doorpos [level] == (DOOR1 | DOOR2)))
...
BYTE she_parkdoor(void){
	if((!(p.cooperate_sel1 & COOP_THROUGH_DOOR)) || (!she_calldoor_double))
		return 1;
				...
				{//ֻ�ڿ���״̬��������̽�
					...
					if(!check_door_short)
						{//����ƽ�㹦��
							...
							else if((state_ulsi == SWITCH_ON) && (dlshortchktimer < timer) && (ulsi_off_enable))
								{
									if (door_zone & POS_SGM)
										{
											if(!p.relevel)
