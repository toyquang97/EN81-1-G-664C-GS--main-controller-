...
{
	FLASHCFG = (0x05ul << 12) & (~(0x003f)) | 0x003a;		// FLASH access uses 6 cpu clocks
	PCLKSEL0 = 0x00000000;									// Peripheral frequency is the main clock 1/4
	PCLKSEL1 = 0x00000000;
	CLKOUTCFG = 0;											// Output cpu clock
	...
	PLL0CON = 0;											// Close pll0
	PLL0FEED = 0xAA;
	PLL0FEED = 0x55;
	while ((PLL0STAT & (3ul << 24)));				// Wait for completion

	SCS = (SCS & 0x04) | 0x20;								// The main crystal starts from 1~20MHz
	while ((SCS & (1ul << 6)) == 0);					// Wait for the oscillation to stabilize

	CLKSRCSEL = 0x01;													// Select the main crystal oscillator as the PLL0 clock source

	PLL0CFG = (((PLL_NVALUE -1) << 16) | (PLL_MVALUE - 1));	// Set PLL0 coefficient
	PLL0FEED = 0xAA;
	PLL0FEED = 0x55;

	PLL0CON = 1;																// Open pll0, but not connected
	PLL0FEED = 0xAA;
	PLL0FEED = 0x55;

	CCLKCFG = (FCCO / FCCLK) - 1;							// Set the division factor of PLL->CPU clock
	while ((PLL0STAT & (1ul << 26)) == 0);					// Wait for PLL lock
	while (((PLL0STAT & 0x00007FFF) != (PLL_MVALUE - 1))	// Read back the M/N coefficient
		&& (((PLL0STAT & 0x00FF0000) >> 16) != (PLL_NVALUE - 1)));

	PLL0CON = 3;
	PLL0FEED = 0xAA;
	PLL0FEED = 0x55;
	while ((PLL0STAT & (1ul << 25)) == 0);					// Wait for the PLL to connect to the CPU clock
}

/****************************************************
* Function name: LpcPinInit
* Function: Initialize I/O function
* Entry: None
* Reference: None
* Return value: None
****************************************************/
void LpcPinInit (void)
{
	...
	PINSEL10 = P10_3_FUNC;					// TPIU禁止, P2.2~p2.6不用作Trace Function
	...
	FIO1SET = PIO_8 | PIO_9 | PIO_14 | PIO_15 | PIO_20 | PIO_23;		//GCAN, 数码管片选
	...
	IO2IntEnr = (0x01ul << 9) | (0x01ul << 12) | (0x01ul << 13);			//Safety loop interrupt using rising edge
	IsrSet(NVIC_EINT3, PRIO_ZERO);		// I/O interrupt and EXT3 share interrupt vector

	EXTMODE = (0x01 << 1);		// External interrupt 1 edge trigger
	EXTPOLAR = (0x00 << 1); 	// External interrupt 1 falling edge trigger
	EXTINT = (0x01 << 1); 	// Clear interrupt flag
	IsrSet(NVIC_EINT1, PRIO_TWO);
}

/****************************************************
* Function name: LpcSpiInit
* Function: Initialize SPI interface for accessing EEPROM and SD
* Entry: None
* Reference: None
* Return value: None
****************************************************/
...
/****************************************************
* Function name: LpcUartInit
* Function: Initialize serial output
* Entry: None
* Reference: None
* Return value: None
****************************************************/
...
/****************************************************
* Function name: LpcQeiInit
* Function: Initialize QEI interface
* Entry: None
* Reference: None
* Return value: None
****************************************************/
...
//Some parameter changes made for the can parameter
//CANA
	rca = 0;
	roa = 0;
	ria = 0;
...
// PR17 Test runs
	...
	test_starttime = 0;
	test_endtime = 0;
	test_interval = 20;		//Running interval is 20s
...
void InitParameter (void)
{
	...
	for (i = 0; i < MAX_HSE_IN; ++i)
	{
		...
		if ((p.hse_inpar [i][IO_BASIC_FUNC] == BRAKE_CHECK) &&
				...
				((p.hse_inpar[i][IO_ENABLE] == ENABLE) || p.ucmp_enable))		//When the UCMP function is present, be sure to check the brake feedback and contactor feedback
