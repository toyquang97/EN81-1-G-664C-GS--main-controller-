...
void init_can_a (void){
	...
	CAN1IER = 0x48Ful;				//接收中断，发送缓冲1中断，错误报警中断，数据超载中断，总线错误中断，发送缓冲3中断
	CAN1MOD &= ~0x01ul;
	CAN1CMR = CMD_CLR_DATAOVERRUN;
	SETENA0 = ISE_CAN;

	CAN1TFI2 = (0x01ul << 16) | SEND_PRIORITY_HIGH;		//心跳发送设置(使用 buf2 )
