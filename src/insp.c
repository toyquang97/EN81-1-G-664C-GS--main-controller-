...
void inspection_state (void)
{
	...
	{		
		...
		if (hse_state != H_INSP)							/* state changed						*/
		{
			...
			if ((p.setup_ready) && (p.cooperate_sel1 & COOP_INSP_RETURN))
				insp_return = 1;
			return;								/* go to new state					*/
		}

		switch (insp & 0x07)							/* set max. speed depennding from direction		*/
		{
			case (3):									/* up direction						*/
				insp_dir = DIR_UP;						/* set ils_active direction				*/
			...
			case (5):									/* down direction						*/
				insp_dir = DIR_DN;						/* set inspection direction				*/
		...
		switch (insp & 0x07)							/* handle pushes						*/
		{
			case (3):									/* up direction						*/
			case (5):									/* down direction						*/
				if (!btnpressed)
				...
				if (drive_state == D_STOPPED)			/* drive in stopped state				*/
				{
					if (p.motor_tuned)
					{
						...
						{
							if ((!limitenable) && (!limitact) && (!ils_active) && insp_dir)
							{
								start_drive (insp_dir);					/* start drive						*/
